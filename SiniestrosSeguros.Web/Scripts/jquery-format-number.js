...
(function () {
    ...
    (a = $(document)).on("change", '.formatoMonto', function (a) {
        ...
        a.on("keypress", '.formatoEntero', function (a) {
            var evt = a, thisT = this, parteEntera = 9, parteDecimal = 2;
        ...
        a.on("keypress", '.formatoMonto', function (a) {
        ...
        a.on("paste", '.formatoMonto,.formatoEntero', function (a) {
            return false
        }),
        ...
        a.on('change', '.hasDatepicker', function (a) {
            var minDay = 31;
            var minMes = 12;
            var minYear = 2099;
            var valor = $(this).val().trim();
            if (valor != '') {
                var expr = new RegExp("^(0[1-9]|[1-3][0-9]){1,2}\/((0[1-9])|(1[0-2]))\/[0-9]{4,}$")

                if (!expr.test(valor)) {
                    $(this).val('')
                    toastr.error('Formato de Fecha no valido');
                } else {

                    var bit = false;
                    var day = valor.split('/')[0];
