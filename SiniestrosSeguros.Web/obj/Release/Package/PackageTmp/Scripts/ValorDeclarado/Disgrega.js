...
function inicializaTablaDetalleDisgrega() {
    ...
    table = $("#tbValorDeclaradoDetalleDisgregado").DataTable({
        ...
        columns: [
            { "data": "IdValorDeclaradoDetalleDisgregado" },
        ...
        if (!$("#txtImporte").prop('disabled')) {
            ...
            table.rows().every(
                function (rowIdx, tableLoop, rowLoop) {
                    ...
                    if ($("#selTipoValorDeclarado option:selected").val() ==dataRow.IdTipoValorDeclarado) {
                        importe += parseFloat(siniestros.resetComas(dataRow.Importe));
                    }
                })
...
function actualizaTipoPoliza() {
    $.ajax({
        type: 'GET',
        url: 'ValorDeclarado/ObtenerTiposPolizas',
