...
function agregarRegistro() {
    ...
    //    if (($("#ValorDeclaradoDetalle_ImporteValorDeclarado").val() == undefined ||
    //        $("#ValorDeclaradoDetalle_ImporteValorDeclarado").val() == '')
    //        && $("#ValorDeclaradoDetalle_ImporteValorDeclarado").is(':disabled') == false
    //        && $("#IdTipoValorDeclarado option:selected").text().trim() == 'Cantidad de Vehículos') {

    //        erroresDetalle.push("- Debe seleccionar un importe");
    //    }
    //    if ($('#IdVerDetalle').is(':disabled') == true) {

    //        erroresDetalle.push("- No ha cargado ningun archivo para este tipo de valor declarado");
    //    }
    //}
    var existe = 1;
    table.rows().every(function (rowId, tableLoop, rowLoop) {
...
function agregarRegistroOK() {
    ...
    var detalle = {
        ...
        Cantidad: $("#ValorDeclaradoDetalle_Cantidad").val(),
...
function getInsDetalleValorAgregado(frm) {
    ...
    table.rows().every(function (rowId, tableLoop, rowLoop) {
        ...
        $obj.CantidadDetalle = siniestros.resetComas(data.Cantidad);
        $obj.IdMonedaDetalle = data.IdMoneda;
...
function CrearArchivoErrores(RegistrosError) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            window.location = "DescargarArchivoError?NombreArchivo=Errores&NombreAsignado=Errores&Formato=.txt";
        },
        failure: function (data) {
            $("#divError").empty().html(data);
            $("#divError").show();
